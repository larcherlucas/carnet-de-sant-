...
<template>
  <div class="space-y-4">
    <div 
      v-for="vaccine in sortedVaccines" 
      :key="vaccine.id" 
      ...
      <div class="flex justify-between items-start">
        <div class="space-y-2">
          ...
          <p 
            :class="[
              'text-sm font-medium', 
              getStatusColor(getVaccineStatus(vaccine.nextDate))
            ]"
          >
            Prochain rappel : 
            {{ dayjs(vaccine.nextDate).format('DD MMMM YYYY') }}
            ({{ getVaccineStatus(vaccine.nextDate) }})
          </p>
          <p class="text-sm text-gray-600">
            Vétérinaire : {{ vaccine.veterinarian }}
          </p>
          <p v-if="vaccine.description" class="text-sm text-gray-500">
            {{ vaccine.description }}
          </p>
        </div>
